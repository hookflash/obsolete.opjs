require([
  'modules/peer', 'modules/transport', 'modules/layout', 'modules/login', 'modules/user-view',
  'modules/incoming-call', 'modules/util', 'q', 'jquery', 'modules/database'
  ], function(Peer, Transport, Layout, Login, UserView, IncomingCall, util, Q, $, db) {
  'use strict';
  ...
  var transport = new Transport({
    ...
    update: function(msg) {
      ...
      if (!peer) {
        ...
        console.warn('ICE: Rejecting candidate--unknown peer:', msg, 'Location IDs:', Object.keys(peers));
    ...
    getMessage: function(request){
      ...
      layout.trigger('on-chat-message', blob, locationID);
  ...
  loginView.then(function(result) {
      ...
      var DataBase = new db(result.provider);

      return DataBase.then(function(userData){
        user.set(userData.user);

        window.$username = user.get('name');

        $.get('/set-name?username=' + user.getContactId());
        ...
        layout.render();
        layout.setContacts(DataBase, contacts, userData.contacts);
      });
    })
    .then(function() {
