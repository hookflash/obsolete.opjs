define([
  ...
  function Peer(context, finder) {
    ...
    self.on("message", function(location, message) {
      ...
      if (message.request.$handler === "peer-common" && message.request.$method === "peer-publish") {
  ...
  Peer.prototype.publish = function(publication) {
    return this._finder._account._pubrepo.publish(this, publication);
  }
