define([
   ...
   */
  function Account(context, stack, location, options) {
    var self = this;
    ...
    // TODO: Use supplied `options.contactID`
    self._contactID = "peer://" + (options.domain || Util.getHost()) + "/" + Util.randomHex(32);
    self._identityID = options.identityID || self._contactID;

    self._location.setContact(self._contactID);

    // First we initialize the `Bootstrapper` and wait until it is ready.

    self._bootstrapper = new Bootstrapper(context, self, self._identityID);
