...
define([
  ...
  AccountFinder.prototype.findPeer = function(contact) {
    ...
    return peer.ready().then(function() {
      return self._session.findPeer(peer).then(function(peer) {
...
console.log("FOUND PEER!");
  ...
  AccountFinderSession.prototype.findPeer = function(peer) {
    var self = this;
    ...
    var findSecretProof = self._account._peerFiles.makeFindSecretProof();

    return self._connection.makeRequestTo("peer-finder", "peer-location-find", {
      "findProofBundle" : self._account._peerFiles.signBundle("findProof", {

        // TODO: Is this in fact the session ID?
        "$id": self._id,
      ...
      Assert.equal(typeof result.locations, "object");
      Assert.notEqual(typeof result.locations.location, "undefined");
      var location = result.locations.location;
      ...
      if (location && location.length > 0) {
        location.forEach(function(location) {
          if (location && location.$id && location.contact) {
