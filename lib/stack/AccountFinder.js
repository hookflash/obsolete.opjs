...
define([
        ...
        var wsUri = "ws://" + hostname;
          ...
          return connectToFinder(wsUri).then(function(connection) {
            if (connection) {
              ...
              self._session = new AccountFinderSession(self._context, self._account, self._connection, {
                finderID: finder.$id
              });
  ...
  AccountFinder.prototype.findPeer = function(contact) {
    ...
    return peer.ready().then(function() {
      return (new PeerFindSession(self._context, self._session)).find(peer).then(function() {
        ...
        self.log("[AccountFinder] Peer found:", contact);

        // TODO: Wait for all locations to connect.
  ...
  function AccountFinderSession(context, account, connection, options) {
    var self = this;
    ...
    function scheduleKeepalive(timestamp) {
      ...
      self._keepaliveTimer = setTimeout(function() {
        ...
        self._keepaliveTimer = null;
        if (!self._connection) return;
          self.log("[AccountFinderSession] Send keepalive '" + self._id + "'");
    ...
    function destroy() {
      function deleteSession() {
        ...
        self.log("[AccountFinderSession] Deleting '" + self._id + "'");
        return self._connection.makeRequestTo("peer-finder", "session-delete", {
          ...
          self.log("[AccountFinderSession] Deleted '" + self._id + "'");
        }).fail(function(err) {
          // Ignore errors.
        });
      }
      ...
      return deleteSession().then(function() {
        self._account = null;
        self._connection = null;
        self.log("[AccountFinderSession] Closed '" + self._id + "'");
        self._id = null;
        self.emit("destroy");
      });
    }
    ...
    self.log("[AccountFinderSession] Creating '" + self._id + "'");
    ...
    self._ready = self._connection.makeRequestTo("peer-finder", "session-create", {
      ...
      Assert.isNumber(result.expires);
      scheduleKeepalive(parseInt(result.expires));
      self.log("[AccountFinderSession] Created '" + self._id + "'");
  ...
  PeerFindSession.prototype.find = function(peer) {
    ...
    try {
      ...
      function checkIfAllReplied() {
        ...
        for (var locationId in locationsReplied) {
          if (!locationsRequested[locationId]) {
            throw new Error("Got reply from location that was never requested!");
          }
          delete locationsRequested[locationId];
        }
      ...
      function onPeerLocationFindReply(message) {
        if (message.reply && message.reply.$handler === "peer-finder" && message.reply.$method === "peer-location-find") {
          ...
          locationsReplied[message.reply.findProofBundle.findProof.requestfindProofBundleDigestValue + ":" + message.reply.findProofBundle.findProof.location.$id] = message.reply.findProofBundle.findProof;
      ...
      // Wait for all locations to respond with timeout of 30 seconds.
      ...
      var location = self._session._account.getLocation().getPayload();
      location.candidates = {
        "candidate": {
          "transport": "webSocket",
          "ip": "localhost",
      ...
      var findSecretProof = self._session._account._peerFiles.makeFindSecretProof();
      ...
      self._session._connection.makeRequestTo("peer-finder", "peer-location-find", {
        ...
        Util.arrayForPayloadObject(result.locations && result.locations.location).forEach(function(location) {
          ...
          locationsRequested[findProofBundle.signature.digestValue + ":" + location.$id] = true;
        });
