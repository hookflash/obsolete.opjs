define([
  ...
  Bootstrapper.prototype.getServices = function() {
    ...
    // @see http://docs.openpeer.org/OpenPeerProtocolSpecification/#BootstrapperServiceRequests-ServicesGetRequest
      ...
      Assert.equal(typeof response.body.result.services, "object");
      Assert.equal(Array.isArray(response.body.result.services.service), true);

		  self._services = response.body.result.services.service;
  ...
  Bootstrapper.prototype.getFinders = function() {
    ...
    return this.getServices().then(function(services) {
      ...
      return new Request({
        ...
        Assert.equal(typeof response.body, "object");
        Assert.equal(typeof response.body.result, "object");
        Assert.equal(typeof response.body.result.finders, "object");
        Assert.equal(Array.isArray(response.body.result.finders.finderBundle), true);

        // TODO: Verify signatures of finders against info in `services`

        self._finders = response.body.result.finders.finderBundle;
  ...
  Bootstrapper.prototype.getCertificates = function() {
    ...
    return this.getServices().then(function(services) {
      ...
      return new Request({
        ...
        Assert.equal(typeof response.body, "object");
        Assert.equal(typeof response.body.result, "object");
        Assert.equal(typeof response.body.result.certificates, "object");
        Assert.equal(Array.isArray(response.body.result.certificates.certificateBundle), true);

        // TODO: Verify signatures of certificates against info in `services`

        self._certificates = response.body.result.certificates.certificateBundle;
