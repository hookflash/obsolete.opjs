define([
   ...
   * Bootstrapper.
   */
  function Bootstrapper(account, id) {
  	var self = this;
  ...
  Bootstrapper.prototype.getServices = function() {
    ...
    return Request.makeRequestTo(url, "bootstrapper", "services-get").then(function(result) {
  ...
  Bootstrapper.prototype.getFinders = function() {
    ...
    return this.getServices().then(function(services) {
      ...
      return Request.makeRequestTo(url, "bootstrapper-finder", "finders-get", {
        "servers": 2
      }).then(function(result) {

        Assert.equal(typeof result.finders, "object");
        Assert.equal(Array.isArray(result.finders.finderBundle), true);
        Assert.equal(result.finders.finderBundle.length > 0, true);

        var finders = [];
        result.finders.finderBundle.forEach(function(bundle) {
  ...
  Bootstrapper.prototype.getCertificates = function() {
    ...
    return this.getServices().then(function(services) {
      ...
      return Request.makeRequestTo(url, "certificates", "certificates-get").then(function(result) {
  ...
  Bootstrapper.prototype.getSalts = function(number) {
    ...
    return this.getServices().then(function(services) {
      ...
      return Request.makeRequestTo(url, "peer-salt", "signed-salt-get", {
        "salts": number || 1
      }).then(function(result) {

        Assert.equal(typeof result.salts, "object");
        Assert.equal(Array.isArray(result.salts.saltBundle), true);
        Assert.equal(result.salts.saltBundle.length > 0, true);

        var salts = [];
        result.salts.saltBundle.forEach(function(bundle) {
