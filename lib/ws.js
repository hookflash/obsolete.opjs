...
define([
			...
			try {
				...
				self._socket.onopen = function(evt) {
					try {
						...
						self.error("Error delivering websocket `open` event", err.stack);
						throw err;
					}
				};
				self._socket.onclose = function(evt) {
					try {
						...
						self.error("Error delivering websocket `close` event", err.stack);
						throw err;
					}
				};
				self._socket.onmessage = function(evt) {
					try {
						...
						if (self._context._debug) {
							self.log("[ws] Received (raw):", evt.data);
						}						
						...
						self.error("Error delivering websocket `message` event", err.stack);
						throw err;
					}
				};
				self._socket.onerror = function(evt) {
					try {
						self.error("Socket error", evt);
						// TODO: Put some info from `evt` into error message below.
						self.emit("error", new Error("Socket error"));
					} catch(err) {
						self.error("Error delivering websocket `error` event", err.stack);
						throw err;
					}
				};
			} catch(err) {
	...
	WS.prototype.send = function(message) {
		try {
			...
			this.error("Error sending websocket message", err.stack);
			throw err;
		}
	}
	...
	WS.prototype.close = function() {
		try {
			...
			this.error("Error closing websocket", err.stack);
			throw err;
		}
	}
	...
	WS.prototype.makeRequestTo = function(handler, method, extra) {
		var self = this;
	  	try {
			...
			var id = Util.randomHex(32);
	  		...
	  		self.once("result:" + id, function(result) {
	  			try {
	  				// Got response after 
	  				if (!responseTimeout) {
	  					self.warn("Got response from '" + self._wsUri + "' after timeout!");
	  					return;
	  				}
	  				clearTimeout(responseTimeout);
	  				responseTimeout = null;
					if (self._context._debug) {
						self.log("[ws] Received [" + id + "]:", result);
					}
			...
			if (self._context._debug) {
				self.log("[ws] Send (object) [" + id + "]:", payload);
			}
			payload = JSON.stringify(payload);
			if (self._context._debug) {
				self.log("[ws] Send (json) [" + id + "]:", payload);
			}
	  		self.send(payload);
