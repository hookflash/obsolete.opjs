...
define([
	...
	function encryptWithPublicKey(publicKey, data) {
		return binaryToBase64(PKI.rsa.encrypt(data, publicKey, true));
	}

	function decryptWithPrivateKey(privateKey, data) {
		var decrypted = PKI.rsa.decrypt(base64ToBinary(data), privateKey, false, false);
// WORKAROUND: https://github.com/digitalbazaar/forge/issues/37
		var decryptedArray = UTILS.stringToArray(decrypted);
		while(true) {
			if (decryptedArray[0] === 0) {
				decryptedArray.shift();
			} else {
				break;
			}
		}
		return decryptedArray.map(function(code) {
            return String.fromCharCode(code);
        }).join('');
// /WORKAROUND
   	}
